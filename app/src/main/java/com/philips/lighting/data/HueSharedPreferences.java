...
public class HueSharedPreferences {
   ...
   public boolean setLastConnectedIPAddress(String ipAddress) {
    ...
    public int getFailureCount() {
        return mSharedPreferences.getInt(FAILURE_COUNT, 0);
    }

    public boolean setFailureCount(int failCount) {
        mSharedPreferencesEditor.putInt(FAILURE_COUNT, failCount);
        return (mSharedPreferencesEditor.commit());
    }

    public String getFragment() {
        String username = mSharedPreferences.getString(FRAGMENT, "");
        return username;
    }

    public boolean setFragment(String fragment) {
        mSharedPreferencesEditor.putString(FRAGMENT, fragment);
        return (mSharedPreferencesEditor.commit());
    }
}
