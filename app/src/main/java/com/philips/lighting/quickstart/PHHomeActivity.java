...
public class PHHomeActivity extends Activity implements OnItemClickListener {
    ...
    private Button btnRefreshBridgeList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btnRefreshBridgeList = (Button) findViewById(R.id.btnRefreshBridgeList);
        btnRefreshBridgeList.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        Log.d(TAG, "List size" + accessPointList.getCount());
    ...
    private PHSDKListener listener = new PHSDKListener() {
        ...
        public void onAccessPointsFound(List<PHAccessPoint> accessPoint) {
            Log.d(TAG, "Access Points Found. " + accessPoint.size());
            ...
            if (accessPoint != null && accessPoint.size() == 1) {
                Log.d(TAG, "Attempting to connect to single bridge");
                Looper.prepare();
                connectToBridge();
            }
        }
        ...
        public void onError(int code, final String message) {
            ...
            if (code == PHHueError.NO_CONNECTION) {
                ...
                final String errMessage = "Error... Please make sure that wifi is enabled and that you are in range of your Hue bridge";
                PHHomeActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        PHWizardAlertDialog.showErrorDialog(PHHomeActivity.this, errMessage, R.string.btn_ok);
                    }
                });
            } 
                ...
                if(prefs.getFailureCount() >= 2) {
                    prefs.setLastConnectedIPAddress(null);
                    prefs.setUsername(null);
                    doBridgeSearch();
                } else {

                    // Increase Faliure count
                    prefs.setFailureCount(prefs.getFailureCount() + 1);

                    final String errMessage = "Bridge not responding... Please make sure that wifi is enabled and that you are in range of your Hue bridge";
                    PHHomeActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            PHWizardAlertDialog.showErrorDialog(PHHomeActivity.this, errMessage, R.string.btn_ok);
                ...
                if (!lastSearchWasIPScan) {  // Perform an IP Scan (backup mechanism) if UPNP and Portal Search fails.
                    ...
                    final String errMessage = "Bridge not responding... Please make sure that wifi is enabled and thatyour Hue bridge is properly configured";
    ...
    public void populateList() {
        ...
        if (lastIpAddress !=null && !lastIpAddress.equals("")) {
            ...
            Log.i(TAG, "First time use, so perform a bridge search");
            doBridgeSearch();
        }
    }
    ...
    public void connectToBridge() {
        PHAccessPoint accessPoint = (PHAccessPoint) adapter.getItem(0);
